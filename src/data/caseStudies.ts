...
const qwenLongContext = { title: 'Qwen 14B 1M 모델 설명', url: 'https://huggingface.co/Qwen/Qwen2.5-14B-Instruct-1M' }
...
export const caseStudies: Record<string, CaseStudy> = {
    ...
    decisions: [
      {
        ...
        approach: '첫 액션 레이어를 6축에 맞춰 초기화하고 16×6 명령을 4×24로 재배열했습니다. 영상 VAE가 시간을 4배 압축하므로 연속된 네 명령을 하나의 잠재 시점에 대응시킨 것입니다. Cosmos-Predict2.5-2B는 동결하고 액션 임베더와 LoRA만 학습해, 사전학습된 영상 표현을 유지하면서 로봇 명령과의 관계를 조정했습니다.',
        ...
        implementation: ['첫 액션 레이어 재초기화·두 번째 레이어 가중치 재사용', 'LoRA rank 32 · attention·MLP에 적용', 'VAE 잠재값 104,828개를 미리 계산해 학습 중 반복 인코딩 생략', '16×6 관절 명령의 시간 순서를 유지하며 4×24로 재배열'], source: solution,
      },
      {
        ...
        approach: '연장한 체크포인트를 제외하고 95,500회 시점으로 되돌렸습니다. 학습량과 데이터 샘플링을 한꺼번에 바꾸면 점수 변화의 원인을 구분할 수 없어, 학습 길이만 변경한 결과를 별도로 비교했습니다. 균등 샘플링 실험도 개선되지 않아 최종 구성에서 제외했습니다.',
      ...
      },
      {
        ...
        approach: '설정별 점수와 시간을 비교해 오토가이던스 0.6과 30-step FlowUniPC를 선택했습니다. 샘플링 스텝을 늘리는 변경은 영상 한 개뿐 아니라 216개 전체의 실행 시간으로 판단했습니다. 액션 CFG와 후보 평균은 최종 구성에서 제외하고, 추가 계산을 샘플링 품질에 사용하는 구성을 채택했습니다.',
    ...
    decisions: [
      {
        ...
        approach: 'AnyLabeling에서 box·rail 두 클래스의 polygon을 자동 사전 라벨링한 뒤 수동 보정했습니다. 꼭짓점은 윗면·밑면, 앞·뒤, 좌·우의 8개 위치를 고정하고 보이는 점을 직접 클릭했습니다. 박스 ID로 윤곽과 꼭짓점 라벨을 연결해 한 프레임에 여러 박스가 있어도 서로 다른 물체의 좌표가 섞이지 않게 했습니다. 가려진 꼭짓점은 좌표를 추정해 채우지 않고 미라벨 상태로 남겼습니다.',
      ...
      },
      {
        ...
        approach: '실제 폭 62.3cm인 레일의 양쪽 경계로 위치별 단위 비율을 계산했습니다. 같은 길이도 카메라에서 멀어지면 작게 보이므로, 측정 위치의 레일 폭을 기준으로 픽셀을 실제 길이로 환산했습니다. 크기에는 실루엣·꼭짓점·시간에 따른 변화를 사용하고, 개수는 검출 시계열·원거리 확대·역방향 추적 통계로 별도 회귀했습니다.',
        expectation: '크기 오차와 개수 오차를 구분하고, 한 프레임에서 보이지 않는 박스를 다른 시점의 관측으로 보완하려 했습니다. 추적 ID 개수만 세는 방식에 의존하지 않으면서, 크기를 추정한 목록의 길이도 실제 통과한 박스 수에 맞추고자 했습니다.',
      ...
      },
      {
        ...
        approach: '학습 스크립트가 추론 main.py의 특징 함수를 재사용하게 했습니다. 크기를 부피 스케일과 형태 비율로 분리해 전체적인 크기와 가로·세로·높이의 비율을 따로 보정했습니다. 원본 모델과 ONNX 출력의 차이도 검증해, 모델 변환 뒤에 생기는 차이를 예측 모델의 오차와 구분했습니다.',
  ...
  'undergraduate-research-smishing': {
    cardLine: '한국어 스미싱 데이터셋을 구축하고 분류·설명 모델을 비교해 논문으로 정리했습니다.',
    lede: 'AIF.Lab 학부연구생으로 한국어 스미싱 데이터셋 구축과 탐지 모델 비교 연구를 수행하고 논문을 작성했습니다.',
    ownershipNote: '메시지 라벨링·데이터 구조 정의, GPT-4o 판단 근거 생성, 모델 벤치마크와 EXAONE 학습을 수행하고 연구 과정과 결과를 논문으로 정리했습니다.',
    problem: '정상·스미싱 라벨만으로는 판단 이유까지 학습하기 어렵습니다. 실제 정상 알림과 사칭 문자의 표현이 겹친다는 점도 데이터 구성에서 고려해야 했습니다.',
    result: '18,270건의 말뭉치를 구축·공개하고 모델 비교 결과를 논문으로 작성했습니다. 벤치마크에서 CNN Accuracy 0.9908, EXAONE 0.9261을 기록했습니다.',
    decisions: [
      {
        ...
        approach: 'KISA에서 실제 공격 사례로 분류된 문자와 개인 모바일 기기·공공자료 등에서 수집한 정상 문자를 구분했습니다. 광고·마케팅을 스미싱과 구별하고, 분류 라벨과 별도로 8개 메시지 유형을 정리했습니다. 정상 결제 안내와 결제 사칭 문자처럼 주제가 같아도 라벨이 다른 사례를 대조할 수 있도록 구성했습니다. 논문에는 이러한 수집 기준과 유형별 예시를 함께 제시했습니다.',
      ...
      },
      {
        ...
        approach: 'GPT-4o 시스템 프롬프트에 금융 보안 전문가 역할, 스미싱 정의, 기관·자녀 사칭과 미납 벌금 사례, 예방 수칙을 넣었습니다. 생성한 판단 근거는 explanation 필드로 저장했습니다. label은 분류 정답, type은 메시지 유형, explanation은 문장별 판단 이유로 나누어 EXAONE 학습 데이터를 구성했습니다. 따라서 사람이 수행한 라벨링과 LLM이 생성한 설명은 서로 다른 데이터 생성 단계입니다.',
      ...
      },
      {
        ...
        approach: 'CNN, FastText, KcELECTRA, EXAONE, Gemini, ChatGPT를 평가하고 Accuracy와 Macro F1을 함께 확인했습니다. 전체 정답 비율뿐 아니라 클래스별 정밀도·재현율을 반영한 F1도 살펴 분류 성능을 비교했습니다. 논문에는 데이터 구축 방법과 이 벤치마크를 함께 정리하고, 설명을 생성할 수 있다는 점을 분류 성능과는 별도의 특성으로 다뤘습니다.',
    ...
    flow: [{ title: '수집·라벨링', detail: '정상·스미싱 분류와 유형 기준 정리' }, { title: '설명 데이터 구축', detail: 'GPT-4o 판단 근거를 학습 데이터에 추가' }, { title: '모델 비교', detail: '분류 모델과 EXAONE의 성능 분석' }, { title: '논문 작성', detail: '데이터 구축 방법·실험 결과·한계 정리' }],
    followUp: { title: '서비스까지 구현', description: '연구 내용을 바탕으로 5명이 함께 메시지를 입력하면 탐지 결과와 판단 이유를 보여주는 서비스까지 구현했습니다.' },
    figures: [
    ...
    decisions: [
      {
        ...
        approach: '생성 모델을 고정하고 TIFF OCR, PDF 정제, Wikipedia 필터링을 거쳐 공통 청크 형태로 통합했습니다. 이미지 문서는 OCR로 텍스트를 얻고, 다른 문서도 같은 검색 단위로 정리해 하나의 인덱스에서 회수하도록 했습니다. 출처 정보를 남겨 검색된 문서를 따로 확인할 수 있게 했습니다. 답이 잘못됐을 때 원문 처리, 청크 구성, 검색 결과를 순서대로 확인할 수 있는 구조입니다.',
      ...
      },
      {
        ...
        approach: '원문과 같은 지식을 묻는 질문을 추가 생성했습니다. 각 질문에서 최대 50개 후보를 회수하고 중복 제거·유사도 필터를 적용한 뒤 BGE Cross-Encoder 리랭킹을 거쳐 최종 5개 이하를 선택했습니다. FAISS는 임베딩으로 후보를 빠르게 찾고, Cross-Encoder는 질문과 각 문단을 함께 입력받아 관련성을 다시 계산합니다. 많은 문서를 찾는 단계와 답변에 넣을 근거를 고르는 단계의 역할을 나눴습니다.',
      ...
      },
      {
        ...
        approach: '객관식과 주관식 프롬프트를 분리하고 유형별 허용 출력과 길이를 지정했습니다. 객관식은 선택지 번호, 주관식은 질문에 필요한 단문 답변을 출력하도록 했습니다. 검색 결과 조립과 답변 작성을 별도 단계로 구성해, 같은 근거를 사용하더라도 제출 유형에 따라 출력 규칙을 달리 적용했습니다.',
    ...
    decisions: [
      {
        ...
        approach: '동의어 맵으로 재료명을 정규화하고 bge-m3·ChromaDB로 후보를 검색했습니다. 정규화한 재료 집합의 포함 관계와 활용 수로 재정렬했습니다. 임베딩은 표현이 비슷한 요리를 찾는 데 사용하고, 재료 조건은 실제로 조리 가능한 후보를 고르는 데 사용한 것입니다. 검색 후보 수를 최종 추천 개수보다 넓게 잡아 재료 조건을 적용할 여지도 확보했습니다.',
      ...
      },
      {
        ...
        approach: '타이머, 영상 검색, 자막 읽기, 레시피 검색, 단계 이동의 다섯 도구만 허용했습니다. 구조화된 tool_call을 파싱하고 서버가 실행 결과와 화면 변경 정보를 반환하도록 구성했습니다. 모델은 호출할 기능과 인자를 제안하고, 서버가 허용된 도구인지 확인한 뒤 실제 동작을 처리합니다. 타이머와 단계 이동은 UI action으로 전달해 화면의 상태 변경과 음성 답변이 같은 실행 결과를 사용하게 했습니다.',
      ...
      },
      {
        ...
        approach: '명확한 영상 요청인데 첫 응답에 도구 호출이 없으면 규칙 기반 검색을 사용했습니다. 이 보완 처리는 첫 응답과 명시적인 영상 의도에 한정해 일반 대화에서 불필요한 검색이 실행되지 않도록 범위를 정했습니다. 벡터 DB 부재 시 재료 집합 검색, 음성 출력에는 서비스 설정에 따른 대체 경로를 구성했습니다.',
    ...
    decisions: [
      {
        ...
        approach: '월·행정동별 데이터를 통합해 23개 파생지표를 만들고 다섯 관점으로 구성했습니다. 비율·밀도 지표와 표준화를 사용해 원래 단위가 다른 수치를 비교할 수 있게 했습니다. 대시보드에는 상대 지수와 군집별 특성을 함께 표시해, 종합 순위를 확인한 뒤 해당 지역의 세부 지표까지 내려가 볼 수 있도록 했습니다.',
      ...
      },
      {
        ...
        approach: '각 방식의 점수를 K-means에 사용하고 실루엣 점수를 비교했습니다. 실루엣은 같은 군집 안의 관측치가 얼마나 유사하고 다른 군집과 얼마나 구분되는지를 보는 지표입니다. 군집 수를 k=4로 맞춰 비교한 결과 PCA 점수가 더 높았습니다. 이 결과를 가중치 선택에 활용하되, 이를 지역의 미래 매출을 잘 예측했다는 의미로 해석하지는 않았습니다.',
      ...
      },
      {
        ...
        approach: '서버에서 계산한 지수, 상대 위치, 세부 근거를 구조화해 A.X-4.0-Light에 전달했습니다. 모델은 새 수치를 계산하지 않고 지역의 강점·약점과 주의점을 설명하도록 구성했습니다. 화면에 표시하는 지표와 해설에 전달하는 근거를 연결해, 사용자가 문장 속 해석을 실제 수치와 대조할 수 있게 했습니다. 이 프로젝트에서 XAI는 복잡한 지표를 비전문가가 이해할 수 있는 설명으로 바꾸는 역할입니다.',
  ...
  'hansol-2pass': {
    cardLine: 'VARCO의 중간 출력과 사고 입력·검색 문서를 Qwen 14B 1M에 전달해 안전 대응책을 생성했습니다.',
    lede: '사고 입력과 RAG 검색 문서를 함께 사용하기 위해 VARCO와 장문 입력 모델 Qwen2.5-14B-Instruct-1M을 연결했습니다.',
    ownershipNote: '1인 프로젝트로 유사 QA 검색, VARCO의 중간 정보 정리, Qwen 14B 1M의 최종 답변 생성까지 구성했습니다.',
    problem: '당시 사용하던 모델의 컨텍스트 길이로는 사고 입력과 RAG 문서를 충분히 함께 전달하기 어려웠습니다. 문서를 짧게 자르면 필요한 사고 조건이 빠질 수 있어, 정보 정리와 장문 입력을 함께 다뤄야 했습니다.',
    result: '한솔데코 시즌3 생성 AI 경진대회에서 Private 12위 / 24팀을 기록했습니다.',
    decisions: [
      {
        title: '두 LLM을 연결한 이유', basis: 'design',
        situation: '한 모델에 사고 입력, 검색한 유사 사례, 답변 지침을 모두 넣기에는 컨텍스트 길이가 제한적이었습니다. 검색 내용을 요약하기만 하면 길이는 줄지만, 원문에 있던 세부 조건을 최종 단계에서 다시 참고하기 어렵다는 문제가 남습니다.',
        approach: 'ko-sbert·FAISS로 유사 QA 3개를 찾고 VARCO-8B로 관련 내용을 중간 출력으로 정리했습니다. 최종 생성에는 긴 입력을 처리할 수 있는 Qwen2.5-14B-Instruct-1M을 사용했습니다. VARCO의 요약·압축 결과만 전달하는 것이 아니라 사고 입력과 RAG 문서도 함께 제공해 최종 모델이 원문 맥락과 중간 내용을 같이 사용하도록 했습니다.',
        expectation: '첫 모델은 정보를 정리하고 두 번째 모델은 더 긴 문맥을 받아 대책을 작성하도록 역할을 나누려 했습니다. 요약은 반복 내용을 줄이는 데 사용하되, 요약에서 빠진 조건을 원문 입력과 검색 문서에서 보완할 수 있도록 구성했습니다.',
        outcome: '유사 사례 검색 → VARCO 중간 출력 → 사고 입력·검색 문서와 함께 Qwen 14B 1M에 전달 → 최종 대응책 생성의 흐름으로 대회에 제출했습니다.',
        implementation: ['jhgan/ko-sbert-nli·LangChain·FAISS Top 3', 'VARCO-8B의 중간 답변·관련 내용 요약', '원래 사고 입력 + RAG 검색 문서 + 중간 출력으로 최종 컨텍스트 구성', 'Qwen2.5-14B-Instruct-1M으로 재발방지대책·향후조치 생성'], source: qwenLongContext,
      },
      {
        ...
        approach: '포함할 내용과 제외할 문구를 프롬프트에 명시했습니다. 중간 정보를 정리하는 단계에서는 사고와 유사 사례의 내용을 다루고, 제출 답변 단계에서는 재발방지대책과 향후조치만 작성하도록 역할을 나눴습니다. 긴 문맥을 입력받을 수 있다는 사실이 곧 긴 답변이 필요하다는 뜻은 아니므로, 최종 출력의 범위는 별도로 제한했습니다.',
    ...
    flow: [{ title: '사례 검색', detail: '사고 입력으로 유사 QA 3개 검색' }, { title: '중간 정보 정리', detail: 'VARCO-8B로 관련 내용 요약·압축' }, { title: '컨텍스트 결합', detail: '사고 입력·RAG 문서·중간 출력 함께 전달' }, { title: '최종 생성', detail: 'Qwen 14B 1M으로 대책·향후조치 작성' }],
    overviewFigure: 'two-pass',
    figures: [{ id: 'two-pass', kind: 'image', image: 'assets/project-previews/hansol-2pass-flow.svg', alt: '사고 입력과 RAG 문서를 VARCO 중간 출력과 함께 Qwen 14B 1M에 전달하는 구조', presentation: 'diagram', title: '두 모델의 역할과 최종 입력', caption: 'VARCO는 중간 내용을 정리하고 Qwen 14B 1M은 사고 입력·RAG 문서·중간 출력을 함께 받아 최종 대응책을 생성합니다.', source: { title: '프로젝트 구성 설명', url: hansol } }],
    limitations: ['1M은 사용 모델의 지원 컨텍스트를 뜻하며 실제 제출 입력이 100만 토큰이었다는 의미는 아닙니다.', '두 모델의 품질·지연·메모리를 1-pass와 같은 조건에서 비교한 기록은 없습니다.'],
    reflection: '필요한 문서를 입력에 수용하는 문제와 그 문서에서 필요한 내용을 고르는 문제를 나누어 다뤘습니다. 긴 입력 모델을 사용하는 것과 중간 내용을 정리하는 것은 서로 다른 역할이었습니다. 이후 금융 RAG에서는 입력 전 단계의 후보 검색과 근거 선별을 더 세분화했습니다.',
    scopeNote: '두 모델의 연결 설명은 당시 작업 경험 기준입니다. 현재 공개 main.py는 첫 모델 결과를 저장하며 Qwen 로더는 있지만 두 번째 호출까지 연결되어 있지 않습니다.', resources: [qwenLongContext],
  },
}
